...
- 2026-08-19: Ολοκληρώθηκε η διόρθωση του σφάλματος διεφθαρμένων μελλοντικών ημερομηνιών ΓΕΜΗ (π.χ. έτος 9011, 5015, 3017): α) Καθαρισμός 178 εσφαλμένων εγγραφών από τη βάση, β) Προσθήκη sanitization guard στην `company_defaults` (`incorporation_date > today -> today`), γ) Αυστηρός περιορισμός `incorporation_date__lte=today` στα `_filtered_companies`, `dashboard` metrics & chart data (`gemiapp/views.py`). Όλα τα 44 unit tests περνούν επιτυχώς.
