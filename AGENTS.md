...
- 2026-08-19: Ολοκληρώθηκε το **Performance Optimization Plan**: α) Caching dropdowns `prefectures` & `legal_types` στο Dashboard (`gemiapp/views.py`), β) Exponential backoff rate-limit handling & batch transactions (`transaction.atomic()`) στις εισαγωγές ΓΕΜΗ (`gemiapp/services.py`). Όλα τα 44 unit tests περνούν επιτυχώς.
